...
const FaskesQueue = () => {
  ...
  return (
    <FaskesLayout>
      ...
      {reservations.filter((item) => item.poli_id === parseInt(poli!)).length >
      0 ? (
        <FaskesContainer>
          <Table>
            ...
            <TableBody>
              {reservations
                .filter((item) => item.poli_id === parseInt(poli!))
                .map((item, index) => (
                  <TableRow className="text-center border-black">
                    ...
                    <TableCell className="flex items-center justify-center gap-3">
                      <MainButton
                        text="Check In"
                        onClick={() => checkin(item.reservations_id)}
                        id={`btn-checkin-${item.reservations_id}`}
                      />
                      <Button
                        className="bg-red-500 hover:bg-red-700 duration-500"
                        onClick={() => skip(item.reservations_id)}
                        id={`btn-skip-${item.reservations_id}`}
                      >
                        Skip
                      </Button>
                    </TableCell>
